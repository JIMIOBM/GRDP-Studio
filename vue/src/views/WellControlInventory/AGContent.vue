...
const fittingModeValue = ref('自动拟合')
...
<template>
  <div class="ag-wrap">
    ...
    <div ref="chartAreaEl" class="chart-area">
      ...
      <div v-if="activeChartTab === 'chart' && legendItems.length" class="floating-chart-legend"
        :class="{ dragging: draggingLegend }" :style="legendStyle" @mousedown="startLegendDrag">
        <div v-for="item in legendItems" :key="item.name" class="floating-legend-item"
          :class="{ hidden: isLegendItemHidden(item.name) }" title="点击显示/隐藏" @mousedown.stop
          @click.stop="toggleLegendItem(item.name)">
          <span class="legend-dot"
            :style="{ backgroundColor: isLegendItemHidden(item.name) ? 'transparent' : item.color, borderColor: item.color }"></span>
          <span>{{ item.name }}</span>
        </div>
      </div>
      ...
      <div v-if="activeChartTab === 'table'" class="data-list-panel">
        <el-table :data="tableRows" :loading="tableLoading" size="small" height="100%" border stripe>
          ...
          <el-table-column prop="regularizedPressureDerivativeReciprocal" label="重整压力导数的倒数(1/DER)" min-width="190"
            sortable />
          <el-table-column prop="regularizedPressureIntegralDerivativeReciprocal" label="重整压力积分导数的倒数(1/DERI)"
            min-width="220" sortable />
        </el-table>
      </div>

      <div class="bottom-chart-tabs">
        <button type="button" class="bottom-chart-tab" :class="{ active: activeChartTab === 'table' }"
          @click="showTableTab">
          数据列表
        </button>
        <button type="button" class="bottom-chart-tab" :class="{ active: activeChartTab === 'chart' }"
          :title="chartTabTitle" @click="activeChartTab = 'chart'">
          结果分析图
        </button>
      </div>
    </div>
