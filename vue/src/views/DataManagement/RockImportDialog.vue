...
const downloadTemplate = async (kind) => {
  ...
  try {
    ...
    const worksheet = workbook.addWorksheet('岩石性质数据')
    const headers = ['岩石孔隙度（%）']
    ...
    ElMessage.error(error?.message || '岩石性质数据模板下载失败')
  }
}
...
<template>
  <Teleport to="body">
    <div v-if="modelValue" class="rock-import-overlay" role="presentation" @mousedown.self="close">
      <section
        class="rock-import-dialog"
        role="dialog"
        aria-modal="true"
        aria-labelledby="rock-import-title"
      >
        <header class="rock-import-header">
         <h2 id="rock-import-title">
  {{ importKind === 'result' ? '导入结果分析数据' : '导入岩石性质基础数据' }}
</h2>
          <button type="button" class="rock-import-close" aria-label="关闭导入窗口" @click="close">×</button>
        </header>

        <div class="rock-import-body">
          <div class="rock-import-downloads">
           <button type="button" @click="downloadTemplate(importKind)">
  {{ importKind === 'result' ? '结果数据模板下载' : '数据模板下载' }}
</button>
          </div>

          <div class="rock-import-options" aria-label="导入清洗选项">
          ...
          >
            <input
              ref="fileInput"
              class="rock-import-file-input"
              type="file"
              :accept="acceptedExtensions"
              @change="handleFileChange"
            />
            <el-icon class="rock-import-icon"><Document /></el-icon>
            <template v-if="selectedFile">
              <p class="rock-import-file-name">{{ selectedFile.name }}</p>
              <button type="button" class="rock-import-clear" @click="clearFile">重新选择</button>
            </template>
            <template v-else>
              <p class="rock-import-prompt">点击或将文件拖拽到这里上传</p>
              <p class="rock-import-hint">支持扩展名：.xlsx .xls .csv</p>
            </template>
          </div>
        </div>

        <footer class="rock-import-footer">
          <button type="button" class="rock-import-cancel" @click="close">取消</button>
          <button type="button" class="rock-import-confirm" @click="confirmImport">确定</button>
        </footer>
      </section>
    </div>
  </Teleport>
