...
const isBlasingameNode = (item) => {
  ...
  const nodeName = String(getNodeName(item) || '').trim()

  return BLASINGAME_NODE_TYPES.has(nodeType) || nodeName.toLowerCase() === 'blasingame'
}
...
const findBlasingameNodeByWell = (root, wellName) => {
  ...
  const visit = (item, currentWellName = '') => {
    ...
    const nextWellName =
        nodeType === NODETYPE.NodeType_Well || nodeName === wellName
            ? nodeName
            : currentWellName
...
// 物质平衡后端需要一次性传入全部井名；界面仍只围绕当前选中井展示进度和结果。
const getAllWellNames = () => {
  const names = getWellGroup()?.children
      .map(well => well.wellName || well.label)
      .filter(Boolean) || []
...
const addBlasingameNode = (wellName, rawNode = {}) => {
  const wellItem = ensureWell(wellName, rawNode?.wellId || `well-${wellName}`)
  ...
  const existedIndex = diagnosticNode.children.findIndex(item =>
      item.id === id ||
      item.type === NODETYPE.NodeType_TypicalCurveBlasingame ||
      item.label === 'Blasingame'
  )
...
const applyMaterialBalanceNodes = (node, onlyWellName = '') => {
  if (!node?.subNodes?.length) return
  ...
  node.subNodes.forEach(wellNode => {
    ...
    // 批量计算会返回多口井结果；顶部菜单触发时只把当前井节点新增到左侧树。
    if (onlyWellName && wellName !== onlyWellName) return
...
const applyTypicalCurveNodes = (node) => {
  ...
  const wellNames = new Set(
      (getWellGroup()?.children ?? [])
          .flatMap(well => [well.label, well.wellName])
          .filter(Boolean)
  )
...
const refreshMaterialBalanceNodes = async (onlyWellName = '') => {
  try {
    const res = await nodeApi.getNode(PROJECT_ID, GAS_RESERVOIR_ID, NODETYPE.NodeType_DynamicOriginalGasInplace)
    applyMaterialBalanceNodes(res?.data?.node, onlyWellName)
  } catch {
    // 没有已有物质平衡结果时保持项目树不变。
  }
}
...
const refreshTypicalCurveNodes = async () => {
  try {
    const res = await nodeApi.getNode(
        PROJECT_ID,
        GAS_RESERVOIR_ID,
        NODETYPE.NodeType_ProductivityInstabilityAnalysis
    )

    applyTypicalCurveNodes(res?.data?.node)
  } catch {
    // 没有已有 Blasingame / 典型曲线结果时保持项目树不变。
  }
}
...
const pollMaterialBalanceNode = async (maxRetries = 20, intervalMs = 1500) => {
  for (let i = 0; i < maxRetries; i++) {
    ...
    // calc 上传全部井名后，node/36 返回的是有物质平衡结果的井列表，不再以当前井作为轮询成功条件。
    if (Array.isArray(node?.subNodes)) {
      return node
    }
  }

  throw new Error('物质平衡结果列表加载超时，请稍后刷新查看结果')
}

const pollBlasingameNode = async (wellName, maxRetries = 20, intervalMs = 1500) => {
  for (let i = 0; i < maxRetries; i++) {
    await new Promise(resolve => setTimeout(resolve, intervalMs))
...
const runMaterialBalanceForSelectedWell = async () => {
  ...
  try {
    ...
    const calcWellNames = allWellNames.length ? allWellNames : [targetWellName]

    await materialBalanceApi.calc({
      // 后端按全部井名批量计算；前端只检查并展示当前选中井的物质平衡结果。
      wellNames: calcWellNames,
      gasReservoirType: 1,
      gasReservoirId: Number(GAS_RESERVOIR_ID),
      projectId: Number(PROJECT_ID),
    ...
    applyMaterialBalanceNodes(node, targetWellName)
...
const runBlasingameForSelectedWell = async () => {
  ...
  try {
    ...
    const { rootNode, blasingameNode } = await pollBlasingameNode(targetWellName)
    ...
    const nodeId = blasingameNode?.nodeId || blasingameNode?.id || treeNode?.id

    if (!nodeId) {
      throw new Error('没有找到 Blasingame 对应的 nodeId')
    }

    const resultRes = await typicalCurveApi.getResult(
        PROJECT_ID,
        GAS_RESERVOIR_ID,
        nodeId
    )
...
const openBlasingameNode = async (node) => {
  ...
  try {
    const nodeRes = await nodeApi.getNode(
        PROJECT_ID,
        GAS_RESERVOIR_ID,
        NODETYPE.NodeType_ProductivityInstabilityAnalysis
    )
    ...
    const blasingameNode =
        findBlasingameNodeByWell(rootNode, targetWellName) ||
        node?.treeNode ||
        node?.raw ||
        node

    const nodeId = blasingameNode?.nodeId || blasingameNode?.id || node?.id
...
const handleRefreshTree = (onlyWellName = '') => {
  refreshWaterInvasionNodes()
  // refreshAnalyticMethodNodes()
  refreshMaterialBalanceNodes(onlyWellName)
