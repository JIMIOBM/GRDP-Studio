<script setup>
/** 库 → 损耗评价 → 井筒损耗。本文件包含该页面的参数状态、接口请求、模板和样式。 */
  ...
  <section class="wellbore-loss">
    ...
    <main class="form-canvas" :inert="saving">
      ...
      <template v-if="mode === 'direct'">
        <div class="form-title">请输入井筒损耗气量</div>
      ...
      <template v-else>
        ...
        <section class="volume-section">
          <div class="section-heading">
            ...
            <button class="add-button" type="button" @click="addVolume">＋ 添加井段</button>
          </div>
          <div class="volume-list">
            <div v-for="(item, index) in volumes" :key="item.id" class="volume-row">
              <span class="volume-index">井段 {{ index + 1 }}</span>
