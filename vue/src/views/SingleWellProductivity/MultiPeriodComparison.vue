<script setup>
/** 井 → 单井产能 → 产能对比 → 多周期。本文件包含该页面的参数状态、接口请求、模板和样式。 */
...
const resultTitle = computed(() => [props.wellName, '产能对比', methodType, '分析结果'].filter(Boolean).join('-'))
...
const renderChart = (preserveView = false) => {
  ...
  if (!chart) {
    ...
    chart.on('legendselectchanged', () => {
      if (isMultiPeriod.value) renderChart(true)
    })
  }
  ...
  chart.setOption({
    animation: false,
    title: { text: methodType + '产能对比图', left: 'center', top: 10, textStyle: { fontFamily: 'Microsoft YaHei, Segoe UI, sans-serif', fontSize: 14, fontWeight: 600, color: '#333' } },
...
watch(() => [props.projectId, props.gasReservoirId, props.wellName, methodType], () => {
...
<template>
  <div ref="workspaceEl" class="comparison-workspace">
    <aside class="params-panel water-parameter-theme" :class="{ collapsed: paramsCollapsed }"
      ...
      <template v-else>
        ...
        <div class="panel-body">
          ...
          <fieldset  class="field-section records-section">
            ...
            <div v-else class="record-list">
              <label v-for="record in records" :key="record.key" class="record-row" :class="{ unavailable: !record.available }">
                ...
                <span class="record-info"><span class="record-name" :title="record.recordName">{{ record.recordName }}</span>
                  <span class="record-meta">{{ record.methodName }} · {{ record.dateKind }} {{ record.date || '未记录' }}</span>
                  <span v-if="!record.available" class="record-error">{{ record.unavailableReason }}</span>
                </span>
              </label>
            </div>
          </fieldset>
          <div class="form-actions"><button type="button" class="calculate-button" :disabled="(calculating || loadingRecords)" @click="handleCalculate">{{ calculating ? '计算中…' : '计算' }}</button></div>
        </div>
    ...
    <main class="result-area">
      ...
      <div class="chart-placeholder" :aria-label="'多周期' + '产能对比结果分析图'"><div ref="chartEl" class="comparison-chart" /></div>
    </main>
  </div>
</template>
