...
const wellType = ref('vertical')
const selectedPvtTable = ref('')
const gasType = ref('干气')
const specificGravity = ref('0.7336')
const hydrogenSulfide = ref('14.62')
const carbonDioxide = ref('8.96')
...
const formationPermeability = ref('2.43407')
const formationThickness = ref('316.85')
const skinFactor = ref('0')
const drainageRadius = ref('631.12')
const wellboreRadius = ref('0.09')
const originalFormationPressure = ref('56.34')
const formationTemperature = ref('120')

// 动态产能－不稳定流独立维护的两项输出；按设计不提供无阻流量。
const darcyFlowCoefficient = ref('1.4676E+3')
const nonDarcyFlowCoefficient = ref('2.4004E+0')

const availablePvtOptions = computed(() =>
  props.pvtTableOptions.length ? props.pvtTableOptions : ['PVT性质1']
)

const selectedPvtRecord = computed(() => {
  const index = Number(String(selectedPvtTable.value).match(/(\d+)$/)?.[1])
  return props.pvtRecords.find(record => Number(record?.index) === index) || null
})

watch(selectedPvtRecord, record => {
  const row = record?.gasRows?.[0]
  if (!row) return
  const valueAt = (index, keys) => {
    if (Array.isArray(row)) return row[index]
    return keys.map(key => row?.[key]).find(value => value !== undefined && value !== null)
  }

  gasType.value = String(valueAt(0, ['gasType', 'gas_type']) || gasType.value)
  specificGravity.value = String(valueAt(1, ['specificGravity', 'specific_gravity']) ?? specificGravity.value)
  hydrogenSulfide.value = String(valueAt(2, ['hydrogenSulfide', 'hydrogen_sulfide']) ?? hydrogenSulfide.value)
  carbonDioxide.value = String(valueAt(3, ['carbonDioxide', 'carbon_dioxide']) ?? carbonDioxide.value)
  nitrogen.value = String(valueAt(4, ['nitrogen']) ?? nitrogen.value)
})
...
<template>
  <div class="dynamic-unstable-productivity-wrap">
      ...
      <template v-else>
        ...
        <div v-show="activePanelTab === 'input'" class="panel-body">
          <div class="field-grid">
            <div class="field">
              <label>选择PVT表</label>
              <el-select v-model="selectedPvtTable" size="small" placeholder="请选择" style="width: 100%">
                <el-option v-for="option in availablePvtOptions" :key="option" :label="option" :value="option" />
              </el-select>
            </div>
          </div>
    ...
    <main class="result-area">
      ...
      <div v-show="activeResultTab === 'chart'" class="chart-placeholder" aria-label="动态产能不稳定流结果分析图">
...
.dynamic-unstable-productivity-wrap {
  height: 100%;
  min-height: 0;
  display: flex;
  overflow: hidden;
