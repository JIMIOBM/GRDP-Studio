...
- ECLIPSE DATA inspection v2 adds source-order WELSPECS well names and lexical `DATES`/`TSTEP` schedule records to that overview. It preserves inspection v1 compatibility, has no derived dates, durations, well states or engineering values, and retains no-run/no-deck-modification boundaries. User manual testing completed without reported issues.
