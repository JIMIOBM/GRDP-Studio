-- 理论产能不稳定流表。请在 project_well_theoretical_productivity 主表已经存在后执行。
USE `database`;

ALTER TABLE project_well_theoretical_productivity
...
CREATE TABLE IF NOT EXISTS project_well_theoretical_unstable_calculation (
  ...
  theoretical_productivity_id BIGINT NOT NULL COMMENT '所属理论产能主记录ID',
  unstable_no INT NOT NULL COMMENT '不稳定流内部编号，只增不复用',
  unstable_name VARCHAR(100) NOT NULL COMMENT '显示名称，可重命名',
  pvt_id BIGINT NULL COMMENT '本次计算选择的PVT记录', pvt_name_snapshot VARCHAR(100) NULL COMMENT 'PVT名称快照',
  ...
  CONSTRAINT fk_theoretical_unstable_master FOREIGN KEY(theoretical_productivity_id) REFERENCES project_well_theoretical_productivity(id) ON UPDATE CASCADE ON DELETE CASCADE,
  CONSTRAINT fk_theoretical_unstable_pvt FOREIGN KEY(pvt_id) REFERENCES project_well_pvt(id) ON UPDATE CASCADE ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='理论计算不稳定流计算记录';

CREATE TABLE IF NOT EXISTS project_well_theoretical_unstable_operation (
  id BIGINT NOT NULL AUTO_INCREMENT, unstable_calculation_id BIGINT NOT NULL,
  operation_type ENUM('production','injection') NOT NULL COMMENT '采气或注气', calculated_at DATETIME(3) NOT NULL,
  created_at DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  updated_at DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3),
  PRIMARY KEY(id), UNIQUE KEY uk_theoretical_unstable_operation(unstable_calculation_id,operation_type),
  CONSTRAINT fk_theoretical_unstable_operation FOREIGN KEY(unstable_calculation_id) REFERENCES project_well_theoretical_unstable_calculation(id) ON UPDATE CASCADE ON DELETE CASCADE
...
CREATE TABLE IF NOT EXISTS project_well_theoretical_unstable_input (
  ...
  CONSTRAINT fk_theoretical_unstable_input FOREIGN KEY(operation_id) REFERENCES project_well_theoretical_unstable_operation(id) ON UPDATE CASCADE ON DELETE CASCADE
...
CREATE TABLE IF NOT EXISTS project_well_theoretical_unstable_output (
  ...
  CONSTRAINT fk_theoretical_unstable_output FOREIGN KEY(operation_id) REFERENCES project_well_theoretical_unstable_operation(id) ON UPDATE CASCADE ON DELETE CASCADE
...
CREATE TABLE IF NOT EXISTS project_well_theoretical_unstable_ipr (
  ...
  CONSTRAINT fk_theoretical_unstable_ipr FOREIGN KEY(output_id) REFERENCES project_well_theoretical_unstable_output(id) ON UPDATE CASCADE ON DELETE CASCADE
