...
import com.grdp.studio.wellbore.risk.service.WellboreRiskCalculationService;
...
public class WellboreRiskController {
    private final WellboreRiskCalculationService calculator;
    private final WellboreRiskStorageService storage;
    public WellboreRiskController(WellboreRiskCalculationService calculator,WellboreRiskStorageService storage){this.calculator=calculator;this.storage=storage;}

    @PostMapping("/liquid-loading/calculate") public ApiResponse<LiquidLoadingResult> calculateLiquid(@Valid @RequestBody LiquidLoadingRequest request,
            @RequestHeader(value="token",required=false) String token,@RequestHeader(value="Cookie",required=false) String cookie,
            @RequestHeader(value="Process-Env",required=false) String environment){return ApiResponse.success(calculator.calculateLiquid(request,token,cookie,environment).result());}
    @PostMapping("/liquid-loading/records/save") public ApiResponse<Map<String,Object>> saveLiquid(@Valid @RequestBody LiquidLoadingSaveRequest request,
            @RequestHeader(value="token",required=false) String token,@RequestHeader(value="Cookie",required=false) String cookie,
            @RequestHeader(value="Process-Env",required=false) String environment){return ApiResponse.success(storage.saveLiquid(request,token,cookie,environment));}
    ...
    @PostMapping("/hydrate/calculate") public ApiResponse<HydrateResult> calculateHydrate(@Valid @RequestBody HydrateRequest request){return ApiResponse.success(calculator.calculateHydrate(request));}
