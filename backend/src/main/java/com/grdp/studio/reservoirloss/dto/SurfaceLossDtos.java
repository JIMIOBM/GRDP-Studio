...
/** 储气库地面损耗计算与存储接口的数据结构。 */
public final class SurfaceLossDtos {
    private SurfaceLossDtos() {}
    ...
    public record SurfaceInput(
            ...
            Double gasOilRatio,
            Double inputCondensateLossVolume
    ) {
        /** 只保存当前方式使用的参数，切换方式后不把隐藏的旧输入带入记录。 */
        public SurfaceInput normalized() {
            if (DIRECT_MODE.equals(calculationMode)) {
                return new SurfaceInput(calculationMode, inputLossVolume, null, null, null, List.of(),
                        null, null, null, null, null, null, null, null, null,
                        null, null, inputCondensateLossVolume);
            }
            return new SurfaceInput(calculationMode, null, averageTemperatureK, pressureBefore, pressureAfter,
                    segments, gasType, specificGravity, h2SMoleFraction, co2MoleFraction, n2MoleFraction,
                    modificationMethod, deviationFactorMethod, viscosityMethod, importedFileName,
                    condensateVolume, gasOilRatio, null);
        }

        /** 放空公式与井筒公式相同；只复用计算，不复用井筒的数据库记录。 */
        public WellboreLossDtos.WellboreInput toVentInput() {
            return new WellboreLossDtos.WellboreInput(calculationMode, inputLossVolume,
                    averageTemperatureK, pressureBefore, pressureAfter,
                    segments == null ? null : segments.stream()
                            .map(s -> new WellboreLossDtos.SegmentInput(s.segmentVolume())).toList(),
                    gasType, specificGravity, h2SMoleFraction, co2MoleFraction, n2MoleFraction,
                    modificationMethod, deviationFactorMethod, viscosityMethod, importedFileName);
        }
    }
    ...
    public record Calculation(Long deviationFactorToolboxId,
                              ...
                              double ventLossVolume, double condensateLossVolume) {}
    ...
    public record Detail(RecordSummary summary, SurfaceInput input, Calculation calculation) {}
}
