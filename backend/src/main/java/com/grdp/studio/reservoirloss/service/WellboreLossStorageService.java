...
/** 井筒损耗记录及其放空井段明细的事务化存储。 */
@Service
public class WellboreLossStorageService {
    private final JdbcTemplate jdbc;
    private final TransactionTemplate transaction;

    public WellboreLossStorageService(JdbcTemplate jdbc, PlatformTransactionManager transactionManager) {
        this.jdbc = jdbc;
    ...
    public synchronized RecordSummary save(SaveRequest request) {
        validateCalculation(request.input(), request.calculation());
        // 事务提交完成后才释放本实例锁；多实例争用编号时由唯一索引兜底并重试。
        for (int attempt = 0; ; attempt++) {
            try {
                return transaction.execute(status -> saveInTransaction(request));
    ...
    private Detail mapDetail(ResultSet rs, List<SegmentInput> segments) throws SQLException {
        ...
        WellboreInput input = new WellboreInput(mode, nullableDouble(rs, 6), nullableDouble(rs, 7),
    ...
    private long insertMain(SaveRequest request, int no) {
        ...
        return insertAndReturnKey("""
                ...
                c.wellboreLossVolume());
    }

    private void updateMain(long id, WellboreInput i, Calculation c) {
        jdbc.update("""
                UPDATE project_reservoir_wellbore_loss SET calculation_mode=?,input_loss_volume=?,
                  average_temperature_k=?,pressure_before=?,pressure_after=?,total_segment_volume=?,
                  gas_type=?,specific_gravity=?,h2s_mole_fraction=?,co2_mole_fraction=?,n2_mole_fraction=?,
                  modification_method=?,deviation_factor_method=?,viscosity_method=?,imported_file_name=?,
                  deviation_factor_toolbox_id=?,deviation_factor_before=?,deviation_factor_after=?,
                  wellbore_loss_volume=? WHERE id=?
                """, modeNumber(i.calculationMode()), i.inputLossVolume(), i.averageTemperatureK(),
                i.pressureBefore(), i.pressureAfter(), c.totalSegmentVolume(), i.gasType(), i.specificGravity(),
                i.h2SMoleFraction(), i.co2MoleFraction(), i.n2MoleFraction(), i.modificationMethod(),
                i.deviationFactorMethod(), i.viscosityMethod(), i.importedFileName(),
                c.deviationFactorToolboxId(), c.deviationFactorBefore(), c.deviationFactorAfter(),
                c.wellboreLossVolume(), id);
    }
    ...
    private void replaceSegments(long recordId, WellboreInput input) {
        ...
        for (int index = 0; index < input.segments().size(); index++) {
            jdbc.update("""
                    INSERT INTO project_reservoir_wellbore_loss_segment
                    (wellbore_loss_id,segment_no,segment_volume) VALUES(?,?,?)
    ...
    static void validateCalculation(WellboreInput input, Calculation calculation) {
        if (!Double.isFinite(calculation.wellboreLossVolume()) || calculation.wellboreLossVolume() < 0) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "请先完成井筒损耗计算");
        }
        if (DIRECT_MODE.equals(input.calculationMode())) {
            if (input.inputLossVolume() == null
                    || input.inputLossVolume() < 0
                    || !sameNumber(input.inputLossVolume(), calculation.wellboreLossVolume())) {
                throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "直接输入值与计算结果不一致");
            }
            return;
        }
        if (!FORMULA_MODE.equals(input.calculationMode()) || calculation.deviationFactorToolboxId() == null
                || calculation.deviationFactorToolboxId() <= 0 || calculation.deviationFactorBefore() == null
                || calculation.deviationFactorBefore() <= 0 || calculation.deviationFactorAfter() == null
                || calculation.deviationFactorAfter() <= 0 || calculation.totalSegmentVolume() == null
                || input.segments() == null || input.segments().isEmpty()) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "公式计算结果不完整");
        }
        WellboreLossCalculationService.validateFormula(input);
        double total = input.segments().stream().mapToDouble(SegmentInput::segmentVolume).sum();
        ...
        if (!sameNumber(expected, calculation.wellboreLossVolume())) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "损耗结果与当前参数不一致，请重新计算");
        }
    }
    ...
    private static RecordSummary summary(ResultSet rs) throws SQLException {
        return new RecordSummary(rs.getLong(1), rs.getInt(2), rs.getString(3),
                "wellbore", rs.getTimestamp(4).toLocalDateTime());
    }
