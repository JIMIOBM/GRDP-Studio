 ...
 */
@Component
public class DynamicUnstableSchemaInitializer {
    private final JdbcTemplate jdbc;

    public DynamicUnstableSchemaInitializer(JdbcTemplate jdbc) {
        this.jdbc = jdbc;
    }

    @PostConstruct
    public void initialize() {
        // 单元测试使用的精简 H2 库没有动态产能主表，此时无需创建业务子表。
        Integer masterCount = jdbc.queryForObject("""
                SELECT COUNT(*) FROM information_schema.TABLES
                WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='project_well_dynamic_productivity'
                """, Integer.class);
        ...
        jdbc.execute("""
                CREATE TABLE IF NOT EXISTS project_well_dynamic_unstable_calculation (
                  ...
                  CONSTRAINT fk_dynamic_unstable_master FOREIGN KEY(dynamic_productivity_id)
                    REFERENCES project_well_dynamic_productivity(id) ON UPDATE CASCADE ON DELETE CASCADE,
                  CONSTRAINT fk_dynamic_unstable_pvt FOREIGN KEY(pvt_id)
                    REFERENCES project_well_pvt(id) ON UPDATE CASCADE ON DELETE SET NULL
                ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci
                  COMMENT='动态产能不稳定流计算记录'
                """);
        jdbc.execute("""
                CREATE TABLE IF NOT EXISTS project_well_dynamic_unstable_operation (
                  ...
                  updated_at DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3),
                  PRIMARY KEY(id),
                  UNIQUE KEY uk_dynamic_unstable_operation(unstable_calculation_id,operation_type),
                  CONSTRAINT fk_dynamic_unstable_operation FOREIGN KEY(unstable_calculation_id)
                    REFERENCES project_well_dynamic_unstable_calculation(id) ON UPDATE CASCADE ON DELETE CASCADE
                ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci
                  COMMENT='不稳定流注采方向'
                """);
        jdbc.execute("""
                CREATE TABLE IF NOT EXISTS project_well_dynamic_unstable_input (
                  ...
                  CONSTRAINT fk_dynamic_unstable_input FOREIGN KEY(operation_id)
                    REFERENCES project_well_dynamic_unstable_operation(id) ON UPDATE CASCADE ON DELETE CASCADE
                ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci
        ...
        jdbc.execute("""
                CREATE TABLE IF NOT EXISTS project_well_dynamic_unstable_output (
                  ...
                  CONSTRAINT fk_dynamic_unstable_output FOREIGN KEY(operation_id)
                    REFERENCES project_well_dynamic_unstable_operation(id) ON UPDATE CASCADE ON DELETE CASCADE
                ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci
        ...
        jdbc.execute("""
                CREATE TABLE IF NOT EXISTS project_well_dynamic_unstable_ipr (
                  ...
                  CONSTRAINT fk_dynamic_unstable_ipr FOREIGN KEY(output_id)
                    REFERENCES project_well_dynamic_unstable_output(id) ON UPDATE CASCADE ON DELETE CASCADE
                ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci
    ...
    private void addMasterColumnIfMissing(String column, String definition) {
        ...
        if (count != null && count == 0) {
            ...
            jdbc.execute("ALTER TABLE project_well_dynamic_productivity ADD COLUMN " + column + " " + definition);
        }
    }
}
