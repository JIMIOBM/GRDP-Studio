CREATE TABLE IF NOT EXISTS software_integration_run (
  ...
  active_slot TINYINT GENERATED ALWAYS AS (
    CASE WHEN REGEXP_LIKE(status, '^(CLAIMED|PREPARING|RUNNING_NODAL|RUNNING_PROFILE|RUNNING_NETWORK|RUNNING_ECLIPSE|COLLECTING|CANCEL_REQUESTED)$') THEN 1 ELSE NULL END
  ),
